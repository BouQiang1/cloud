...
class get_loss(nn.Module):
    ...
    # def forward(self, pred, target,smoothing=True):
    def forward(self, pred, target):
        if True:
            ...
            loss = self.loss(pred, target.view(-1))

        return loss
        # B, W, C = pred.size()
        #
        # # BWC -> (BW) x C
        # pred_view= pred.contiguous().view(-1, C)
        # target = target.repeat(1, pred.shape[2])  # Equivalent to tf.tile(labels_2d, (1, logits_op.shape[1]))

        # target = target.argmax(dim=1)
        # total_loss = nn.CrossEntropyLoss(reduction='mean')(pred_view, target.view(-1))
