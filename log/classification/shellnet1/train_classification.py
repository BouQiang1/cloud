...
def main(args):
    ...
    train_dataset = ModelNet40(partition='train', num_points=args.cls_point, cls_path = args.cls_path,fps=args.use_uniform_sample,use_normals=args.use_normals )
    test_dataset =  ModelNet40(partition='test', num_points=args.cls_point, cls_path = args.cls_path,fps=args.use_uniform_sample,use_normals=args.use_normals )
    ...
    # # '''计算模型参数量和Flops'''
    input = torch.randn(1, 1024, 3).type(torch.cuda.FloatTensor)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, label) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader),smoothing=0.9):
            ...
            if args.use_gpu:
                points, label = points.cuda(), label.cuda()
            pred,trans_feat= classifier(points.float()) #trans_feat在分类框架中没有被用到，写在这里是为了统一结构
